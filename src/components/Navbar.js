        ...
        <div className="bg-white z-50 fixed shadow-sm w-full">
            <div className="lg:px-24 py-5 px-8 lg:flex justify-between items-center">
                ...
                <div className={`flex flex-col items-left lg:items-center lg:text-sm text-center lg:flex-row absolute shadow lg:shadow-none lg:static lg:z-auto z-[-1] left-0 w-full px-20 lg:w-auto lg:pl-0 pl:8 transition-all duration-500 ease-in  ${isOpen ? 'top-12 bg-white' : 'top-[-490px]'}`}>
                    <Link to='/' className="mr-8 hover:text-primaryColor duration-300 mt-8 mb-4 lg:my-4 lg:ml-4">صفحه اصلی</Link>
                    <Link to='/testlist' className="mr-5 hover:text-primaryColor duration-300 my-4 lg:ml-4">لیست آزمایشات</Link>
                    <Link to='/blogpage' className="mr-5 hover:text-primaryColor duration-300 my-4 lg:ml-4">وبلاگ</Link>
                    <Link to='/connect' className="mr-5 hover:text-primaryColor duration-300 my-4 lg:ml-96 lg:pl-28 lg:pl-4">ارتباط با ما</Link>
                    <Link to='/javabdehi'><button className=" text-white py-2 lg:px-3 px-28 bg-primaryColor hover:opacity-75 duration-300 rounded my-4 lg:my-0 ">جواب دهی آنلاین</button></Link>
                </div>
            </div>
        </div>
    );
